...
namespace DesignTE.Controller
{
    /// <summary>
    /// Controlller for:
    /// - Launching and Closing of DesignTE application
    ...
    public class MainCtrl : IMainCtrl
    {
        ...
        public const string APPLICATION_NAME = "用例设计";

		#region Launch DesignTE application
        ...
        void mainForm_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                catch(Exception exp)
                {
                    ...
                    System.Diagnostics.Trace.TraceWarning(DateTime.Now.ToString() + ": 无法加载最后打开的文件！ " + exp.Message);
                }
            }
        ...
        #endregion Launch DesignTE application

        #region Shutdown DesignTE application
        ...
        #endregion Shutdown DesignTE application
        ...
        private ContinueOperation PromptForUnsavedChanges(PersistentTree tree)
        {
            if (tree.IsDirty)
            {
                DialogResult result = MessageBox.Show("需要保存所做的更改吗?", "尚未保存更改",
        ...
        private ContinueOperation PromptForLosingClipboardData()
        {
            if (ClipboardManager.HasCutNode &&
                MessageBox.Show("你会丢失在剪贴板的数据. 确定要退出吗?",
                    "尚未保存更改", MessageBoxButtons.YesNo,
