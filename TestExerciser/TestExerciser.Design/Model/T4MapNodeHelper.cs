    ...
    public class T4MapNodeHelper
    {
        ...
        public IEnumerable<PropertyInfo> GetSerializedProperties()
        {
            var props = from p in typeof (MapNode).GetProperties()
                where p.GetCustomAttributes(true).Any(a => a.GetType() == typeof (DesignTE.Serialization.SerializedAttribute))
                select p;
        ...
        {
            var props = from p in typeof(MapNode).GetProperties()
                        let a = (DesignTE.Serialization.SerializedAttribute)Attribute.GetCustomAttribute(p, typeof(DesignTE.Serialization.SerializedAttribute))
                        where a != null
