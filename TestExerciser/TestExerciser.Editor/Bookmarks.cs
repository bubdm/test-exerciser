...
namespace EditorTE
{
    ...
    public class Bookmarks : BaseBookmarks
    {
        ...
        {
            this.tb = tb;
            tb.LineInserted += tLineInserted;
            tb.LineRemoved += tLineRemoved;
        }

        protected virtual void tLineRemoved(object sender, LineRemovedEventArgs e)
        {
        ...
        protected virtual void tLineInserted(object sender, LineInsertedEventArgs e)
        {
            for (int i = 0; i < Count; i++)
                if (items[i].LineIndex >= e.Index)
                {
